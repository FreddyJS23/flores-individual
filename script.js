...
const messageLines = [
  ...
  sino en la intención con que te entrego esto. Y lo que hoy dejo en tus manos nace de mi, de un lugar silencioso donde las cosas simples se vuelven eternas.`,
  
  `Quiero que sientas en este detalle un pedazo de mi verdad: que lo esencial nunca se marchita, a pesar de que el mundo haya muerto, esto, nunca pierde fuerza,
  nunca se olvida. Porque lo esencial vive en mi, en ti, y en todos, como una llama que no se apaga, incluso cuando todo alrededor parece oscurecerse.`,
  
  `Así, sin flores amarillas, pero con todo el corazón, te entrego algo que no busca adornar tus manos, sino abrazar tu alma con este mensaje.`,
];
...
function placeFlowerRandomly(el) {
  ...
  const radius =
    ...
    Math.random() * (Math.min(window.innerWidth, window.innerHeight) / 3);
...
function animateFlowerLifecycle(flowerEl) {
  ...
  const toExit = setTimeout(() => {
    ...
    const anim = flowerEl.animate(
      [
        {
          transform: getComputedStyle(flowerEl).transform || "scale(1)",
          opacity: 1,
        },
...
function spawnParticleDot(x, y) {
  ...
  const anim = node.animate(
    [
      ...
      {
        transform: `translate(${(endX - x) / 2}px,${
          (endY - y) / 2
        }px) scale(1.05)`,
        opacity: 0.9,
        offset: 0.45,
      ...
      {
        transform: `translate(${endX - x}px,${endY - y}px) scale(.9)`,
        opacity: 0,
      },
    ],
  ...
  for (let i = 0; i < count; i++) {
    ...
    const anim = node.animate(
      [
        ...
        {
          transform: `translate(${endX - x}px,${endY - y}px) scale(.6)`,
          opacity: 0,
        },
      ],
      { duration: dur, easing: "cubic-bezier(.22,.9,.3,1)" }
...
function startTypingSequence() {
  ...
  (function nextLine() {
    if (document.hidden) {
      typingPaused = true;
      typingTimer = setTimeout(nextLine, 1000);
      return;
    }
    typingPaused = false;
    typeText(messageLines[idx], typedText, 36, () => {
      // 36ms por carácter (más ágil)
